...
def formate_file_name(file_name):
    ...
    for c in chars:
        file_name.replace(c, "")
    file_name = '@CineFoxHub ' + ' '.join(filter(lambda x: not x.startswith('http') and not x.startswith('@') and not x.startswith('www.'), file_name.split()))
    return file_name
...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            ...
            InlineKeyboardButton('🔍 sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ', url='https://t.me/cinefoxhub'),
            InlineKeyboardButton('🤖 ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ', url='https://t.me/bossvai25')
            ],[
